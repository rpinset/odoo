...
class AccountMove(models.Model):
    ...
    def _l10n_fr_pdp_get_default_notes(self):
        ...
        if tva_group_note := self._l10n_fr_pdp_get_tva_group_note():
            notes['TXD'] = tva_group_note
        return notes

    def _l10n_fr_pdp_get_tva_group_note(self):
        """[BR-FR-05] #TXD# — member of a VAT group ("assujetti unique", Art. 256 C CGI,
        DGFiP use case #29). BT-21 subject code TXD / BT-22 text "MEMBRE_ASSUJETTI_UNIQUE"."""
        self.ensure_one()
        company = self.company_id
        if not company.l10n_fr_is_tva_group_member or not company.l10n_fr_tva_group_id:
            return None
        return "MEMBRE_ASSUJETTI_UNIQUE"
